...
- User needs self-contained architecture diagrams (HTML+CSS+SVG) showing module relationships or deployment topology
...
- For diagram rendering: no external tools needed — the skill produces self-contained `.html` files with inline CSS and SVG that any browser opens
...
Produce self-contained HTML+CSS+SVG diagrams showing module relationships, data flow, and deployment topology. Each diagram is a standalone `.html` file with inline styles and inline SVG — no external libraries, no internet access, no rendering tools. Text-readable for LLMs; visual for humans in any browser.

1. **Context diagram (C4 Level 1)** — show the system as a single box surrounded by users and external systems. Completion criterion: the diagram opens in a browser and shows all external actors and systems from the SRS.
...
See `references/html-svg-diagramming.md` for the HTML scaffold, SVG primitives, worked examples for all five diagram types, and best practices. Use `write_file` to save each diagram as a separate `.html` file alongside the architecture document.

Output: Architecture diagrams (self-contained `.html` files).
...
| 7. Architecture diagrams | Self-contained HTML+CSS+SVG diagrams | `references/html-svg-diagramming.md` |
...
5. **Diagrams requiring external rendering tools.** Architecture diagrams that need Mermaid.js, a PlantUML server, or any external dependency to view create a fragility — if the tool isn't available, the diagram is unreadable. Always produce self-contained HTML+CSS+SVG files that any browser opens with zero dependencies.
...
- [ ] All diagrams are self-contained HTML+CSS+SVG (`.html` files) and open in any browser with zero dependencies
